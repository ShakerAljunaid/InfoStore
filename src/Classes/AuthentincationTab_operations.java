...
public class AuthentincationTab_operations {
    ...
    //Method to show the data existing in the users table
       ...
       //End of showing users method
           ...
           /*//Method to show the data existing in the users table for specific rows
        
           public ResultSet show_Speciusers(int selected_row) throws SQLException
               
       {
           //Call the stored procedure designated for showing subjets
           CallableStatement cstm=dbconn.prepareCall("{call show_selected_user(?)}");
           
           cstm.setInt(1, selected_row);
           //Execute the stored procedure and put the result into data set 
           ResultSet RS=cstm.executeQuery();
           //Return the data set 
           return RS;
